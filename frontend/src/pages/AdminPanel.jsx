...
const downloadCertificate = async (filename) => {
  ...
  if (!res.ok) { alert('Failed to download certificate.'); return; }
              ...
              {selected.certificateFileName && (
                <div style={{ margin: '16px 0' }}>
                  <button onClick={() => downloadCertificate(selected.certificateFileName)}
                    style={{ display: 'inline-block', padding: '10px 18px', background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 2, color: 'var(--gold)', fontSize: 11, letterSpacing: '0.1em', cursor: 'pointer' }}>
